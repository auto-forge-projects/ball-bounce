...
- Express stdout/stderr → container log (`docker logs ball-bounce`); host'ta ek log dosyası tutulmuyor (durumsuz, kalıcı log altyapısı LITE kapsamı dışı).
- Hassas veri loglanmaz: uygulama kimlik/PII/oturum verisi işlemiyor (bkz. `docs/07-security.md`), skor yalnız bellek-içi state'te.
