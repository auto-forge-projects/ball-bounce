...
test('güvenlik header\'ları uygulanır (SEC-2: CSP, nosniff, referrer-policy)', async () => {
  const res = await fetch(`${base}/health`);
...
test('dotfile isteği reddedilir (SEC-3: path traversal / gizli dosya koruması)', async () => {
...
test('bilinmeyen yol 404 döner ve stack trace sızdırmaz (SEC-10)', async () => {
  const res = await fetch(`${base}/olmayan-yol`);
  assert.equal(res.status, 404);
  const text = await res.text();
  assert.doesNotMatch(text, /at\s+\w+\s+\(/); // stack trace izi yok
});
